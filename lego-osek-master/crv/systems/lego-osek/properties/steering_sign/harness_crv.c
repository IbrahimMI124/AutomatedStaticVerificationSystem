...
void steering_sign_step_orig(void);
void steering_sign_step_seu(void);
...
S32 rv_get_motor_steering_speed(void);
void rv_set_motor_steering_speed(S32 v);
...
#define SEU_BT_RIGHT 2
#define SEU_MOTOR_STEER 3
...
static void run_original(const int8_t bt_right_seq[STEPS],
                         const int32_t steering_angle_seq[STEPS],
                         bool violation_seq[STEPS])
{
  steering_sign_monitor_reset_orig();

  for (unsigned t = 0; t < STEPS; t++)
  {
    rv_touch_input = 0u;
    rv_bt_left_input = 0;
    rv_bt_right_input = bt_right_seq[t];
    rv_steering_angle_input = (S32)steering_angle_seq[t];

    TaskControl();

    int32_t steering_angle = (int32_t)rv_get_steering_angle();
    int32_t cmd_steer = (int32_t)((int8_t)rv_bt_right_input);
    int32_t motor_steer = (int32_t)rv_get_motor_steering_speed();

    steering_sign_publish(steering_angle, cmd_steer, motor_steer);
    steering_sign_step_orig();

    violation_seq[t] = steering_sign_monitor_violation_orig();
  }
}

static void run_seu(const int8_t bt_right_seq[STEPS],
                    const int32_t steering_angle_seq[STEPS],
                    bool violation_seq[STEPS])
{
  steering_sign_monitor_reset_seu();
  ...
  for (unsigned t = 0; t < STEPS; t++)
  {
...
#if SEU_TARGET == SEU_BT_RIGHT
    rv_bt_right_input = (t == seu_step) ? flip_s8_bit(bt_right_seq[t], seu_bit) : bt_right_seq[t];
#else
    rv_bt_right_input = bt_right_seq[t];
#endif
...
#if SEU_TARGET == SEU_MOTOR_STEER
    if (t == seu_step)
    {
      rv_set_motor_steering_speed(flip_s32_bit(rv_get_motor_steering_speed(), seu_bit));
    }
#endif
    ...
    steering_sign_publish(steering_angle, cmd_steer, motor_steer);
    steering_sign_step_seu();

    violation_seq[t] = steering_sign_monitor_violation_seu();
  }
}
...
int main(void)
{
  ...
  int8_t bt_right_seq[STEPS];
  int32_t steering_angle_seq[STEPS];

  for (unsigned t = 0; t < STEPS; t++)
  {
#ifdef CBMC
    bt_right_seq[t] = nondet_s8();
    __CPROVER_assume(bt_right_seq[t] >= -100 && bt_right_seq[t] <= 100);
  ...
  run_original(bt_right_seq, steering_angle_seq, violation_seq_orig);

  controller_init_from_snapshot(edc_flag_init, touch_state_init);
  run_seu(bt_right_seq, steering_angle_seq, violation_seq_seu);
