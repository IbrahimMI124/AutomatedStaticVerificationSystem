...
void edc_off_symmetry_step_orig(void);
void edc_off_symmetry_step_seu(void);
...
// SEU targets
#define SEU_MOTOR_LEFT 1
#define SEU_MOTOR_RIGHT 2
#define SEU_EDC_FLAG 3
...
{
  edc_off_symmetry_monitor_reset_orig();
  ...
  for (unsigned t = 0; t < STEPS; t++)
  {
    ...
    edc_off_symmetry_publish(edc_flag, motor_left, motor_right);
    edc_off_symmetry_step_orig();

    violation_seq[t] = edc_off_symmetry_monitor_violation_orig();
  }
}
...
static void run_seu(const uint8_t touch_seq[STEPS],
                    ...
                    bool violation_seq[STEPS])
{
  edc_off_symmetry_monitor_reset_seu();
  ...
  rv_steering_angle_input = (S32)steering_angle_seq[t];
    ...
    edc_off_symmetry_publish(edc_flag, motor_left, motor_right);
    edc_off_symmetry_step_seu();

    violation_seq[t] = edc_off_symmetry_monitor_violation_seu();
  }
}
